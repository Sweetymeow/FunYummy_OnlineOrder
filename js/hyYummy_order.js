...
var orderLists = new Array(50);  

// use to save JSON data - 也许用不上
...
$(function(){
	...
	*/
	...
	$(":submit").click(function(e){
		...
		e.preventDefault();
		orderjson['name']=$("input[name='name']"); //string
		orderjson['cellphone']=$("input[name='cellphone']"); 
		orderjson['email']=$("input[name='email']");
    	orderjson['total'] = parseInt($('div#ordertotal').children("h4").text());
		orderjson['tax'] = parseInt($('div#ordertotal').children("h4").text())*taxpercent;
		orderjson['paystate'] = false;
		orderjson['paytype'] = "cash";
		orderjson['commment'] = "test";
		orderjson['orderday_num'] = orderLists.length; 
		alert("create order json : " + orderjson.length);
		for(var i = 0; i <= orderIndex+1; i++){
			var name = "order_" + orderIndex;
			}
    	 //array
		var jsonOrderInfo = JSON.stringify(orderjson);
		$.post('test.php', jsonOrderInfo
		)
 	 	$( ".result" ).html( data );
});
